---
source_file: "C:\Users\yashb\Pictures\Projects\TestBuddy\Frontend\src\hooks\use-toast.ts"
type: "code"
community: "Auth Controller"
location: "L71"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Auth_Controller
---

# reducer()

## Connections
- [[addToRemoveQueue()]] - `calls` [EXTRACTED]
- [[dispatch()]] - `calls` [EXTRACTED]
- [[use-toast.ts_1]] - `contains` [EXTRACTED]
- [[use-toast.ts_3]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Auth_Controller